...
# --- repo-specific: guard the tasks this repo's own paths would shadow ---------------
#
# `%:` is a match-anything rule, so make only routes a target to the CLI when it cannot
# resolve it as a file. Two task names collide with paths that exist here:
#
#   book     the `book/` directory (the marimo notebooks live under book/marimo/notebooks)
#   license  the `license` file at the repo root
#
# Without this, `make book` and `make license` print "is up to date" and the gate never
# runs -- a silent pass, which is the worst kind. `.PHONY` says these are commands, not
# files, so the catch-all takes them regardless of what is on disk.
#
# `.PHONY` alone is NOT enough, and fails in the direction that looks fine: make never
# applies a match-anything rule to a phony target, so declaring them phony and stopping
# there gets "Nothing to be done for 'book'" -- still a silent pass. They need an explicit
# rule as well, which also beats the pattern rule outright, with `.PHONY` on top so the
# existing `license` file cannot make the target look up to date.
#
# Only names that are BOTH a task and a path belong here. `docs/` is safe: the task is
# `docs-coverage`, and no task is called `docs`.
.PHONY: book license
book license: $(UVX)
	@$(UVX) $(RHIZA_TASK) $@
